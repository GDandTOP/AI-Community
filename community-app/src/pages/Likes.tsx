...
import { getUserLikedPosts } from '@/services/like.service'
...
export function Likes() {
  ...
  const loadLikedPosts = async () => {
    ...
    try {
      setLoading(true)
      setError('')
      const likedPosts = await getUserLikedPosts(currentUser.uid)
      setPosts(likedPosts)
    } catch (err: any) {
  ...
  return (
    <div className="container max-w-4xl py-8">
      <div className="mb-6">
        <h1 className="text-3xl font-bold mb-2">좋아요</h1>
        <p className="text-muted-foreground">
          좋아요한 게시글을 확인하세요 ({posts.length}개)
        </p>
      </div>
